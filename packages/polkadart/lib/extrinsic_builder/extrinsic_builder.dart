...
class ExtrinsicBuilder {
  ...
  /// Build a signed extrinsic from an already-produced signature blob.
  ///
  /// Used for fee estimation (dummy blob) and external/hardware signers.
  EncodedExtrinsic attachSignature({
    ...
    return EncodedExtrinsic.fromSignedData(
      chainInfo,
      SignedData(
        signer: signer,
        signature: signature,
        ...
        callData: callData,
        signingPayload: signingBuilder.createRawPayloadToSign(callData),
      ),
    );
  }
  ...
  /// Unhashed signing payload so a hardware wallet can parse and display the call.
  Uint8List getRawSigningPayload() {
    final signingBuilder = SigningBuilder(
      chainInfo: chainInfo,
      extensionBuilder: _extensionBuilder,
    );
    return signingBuilder.createRawPayloadToSign(callData);
  }

  int get blockNumber => _blockNumber;
  int get eraPeriod => _eraPeriod;
