...
void main() {
  ...
  group('Hardcoded Hex Test Vectors', () {
    ...
    test('opaque 7219-byte signature with variant 0 is not clipped or remapped', () {
      if (!v16Available) return;
      final encoder = ExtrinsicEncoder(v16ChainInfo);
      if (encoder.extrinsicVersion != 5) return;
      ...
      final signature = Uint8List.fromList(List<int>.generate(7219, (i) => i % 256));
      final signedData = SignedData(
        signer: Uint8List(32),
        signature: signature,
        signatureVariant: 0,
      ...
      final encoded = encoder.encodeWithoutPrefix(signedData);
      expect(encoded[34], 0);
      expect(encoded.sublist(35, 35 + 7219), signature);
    });
  });
